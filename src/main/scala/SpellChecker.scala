...
class Trie[Val] {
  ...
  class Node(kids: Map[Char, Node], val contents: Option[Val]) {

    /** *
     * create empty node with empty Map with no character and None as content
     *
     * @return: returns an Map with None and empty map
     */
...
class SpellChecker {
   ...
   */
  def checkWord(inputString: String, dictionaryList: List[String]): String = {
    val loggerObj = new Logger()
    ...
    for (i <- 0 until words.length) {
      if (trieObj.get(zipedWord(i)._1).isDefined) { // extracting string from zipped input
  ...
  def main(args: Array[String]): Unit = {
    ...
    while (flag) {
      ...
      reg.findFirstMatchIn(inputString) match {
        case None =>
          println("The corrected input is  " + spellCheckerObj.checkWord(inputString, dictionaryList))
